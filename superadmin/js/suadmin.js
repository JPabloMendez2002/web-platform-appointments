...
$('#cambiarcontraUsers').on('click', function () {
	var contrasena = $("#contraNewUser1").val();
	var contrasena2 = $("#contraNewUser2").val();
	var id = $("#idPassUser").val();

	var accion = "actualizacontraUsers";
...
$('#actualizaCita').on('click', function () {
	...
	datos.push({ name: 'accion', value: 'actualizaCita' });
...
$('#actualizaPUser').on('click', function () {
	...
	datos.push({ name: 'accion', value: 'actualizaPerfilUSERS' });
	...
	if (IdADU != '') {
		...
		$('#creaCita').click(function (e) { 
			...
			if(!paciente || !fecha){
				...
				datosForm.push({name: 'accion', value: 'creaCita'});
